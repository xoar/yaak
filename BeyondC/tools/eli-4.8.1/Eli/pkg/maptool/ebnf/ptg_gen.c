...
/* Implementation of Pattern DolMod */

typedef struct _SPTGDolMod{
	_PTGProc _print;
	PTGNode p1;
} * _PPTGDolMod;

#ifdef PROTO_OK
static void _PrPTGDolMod(_PPTGDolMod n)
#else
...
#ifdef PROTO_OK
PTGNode PTGDolMod(PTGNode p1)
#else
PTGNode PTGDolMod(p1)

PTGNode p1;
#endif
{
	_PPTGDolMod n;
	n = (_PPTGDolMod)MALLOC(sizeof(struct _SPTGDolMod));
...
/* Implementation of Pattern Conn */

typedef struct _SPTGConn{
	_PTGProc _print;
	PTGNode p1;
} * _PPTGConn;

#ifdef PROTO_OK
static void _PrPTGConn(_PPTGConn n)
#else
static void _PrPTGConn(n)
	_PPTGConn n;
#endif
{
	PTG_OUTPUT_STRING(f, "&");
...
{
	_PPTGConn n;
	n = (_PPTGConn)MALLOC(sizeof(struct _SPTGConn));
	n->_print = (_PTGProc)_PrPTGConn;
	n->p1 = p1;
	return (PTGNode)n;
}
...
#ifdef PROTO_OK
PTGNode PTGRule(PTGNode p1, PTGNode p2, PTGNode p3)
#else
