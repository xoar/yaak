char rcsid_be[] = "$Id: be.c,v 1.4 2013/11/12 18:24:10 profw Exp $";
...
makeCostArray()
{
	...
	for (i = 0; i <= max_erule_num; i++) {
		if (pVector[i]) makeCostVector(1, pVector[i]->rule->delta);
		else makeCostVector(0, 0);
		fprintf(outfile, ", /* ");
...
makeRuleDescArray2()
{
	...
	for (i = 1; i <= max_erule_num; i++) {
		...
		if (pVector[i]) {
			...
			fprintf(outfile, "{");
			fprintf(outfile, "%5d", pVector[i]->rule->lhs->num);
			o = pVector[i]->rule->pat->op;
			fprintf(outfile, "%5d", o ? o->num : 0);
			t = pVector[i]->rule->pat->children[0];
			fprintf(outfile, "%5d", t ? t->num : 0);
			t = pVector[i]->rule->pat->children[1];
			fprintf(outfile, "%5d", t ? t->num : 0);
			fprintf(outfile, "} /* ");
