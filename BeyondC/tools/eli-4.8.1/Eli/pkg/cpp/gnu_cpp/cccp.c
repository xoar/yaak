...
#if PROTO_OK
  int (*func)(U_CHAR *, U_CHAR *, FILE_BUF *, struct directive *);
#else
  int (*func)();
#endif
...
#if PROTO_OK
int do_define(U_CHAR *, U_CHAR *, FILE_BUF *, struct directive *);
int do_if(U_CHAR *, U_CHAR *, FILE_BUF *, struct directive *);
int do_xifdef(U_CHAR *, U_CHAR *, FILE_BUF *, struct directive *);
int do_endif(U_CHAR *, U_CHAR *, FILE_BUF *, struct directive *);
int do_else(U_CHAR *, U_CHAR *, FILE_BUF *, struct directive *);
int do_elif(U_CHAR *, U_CHAR *, FILE_BUF *, struct directive *);
int do_line(U_CHAR *, U_CHAR *, FILE_BUF *, struct directive *);
int do_include(U_CHAR *, U_CHAR *, FILE_BUF *, struct directive *);
int do_undef(U_CHAR *, U_CHAR *, FILE_BUF *, struct directive *);
int do_error(U_CHAR *, U_CHAR *, FILE_BUF *, struct directive *);
#ifdef SCCS_DIRECTIVE
int do_sccs(U_CHAR *, U_CHAR *, FILE_BUF *, struct directive *);
#endif
int do_pragma(U_CHAR *, U_CHAR *, FILE_BUF *, struct directive *);
#else
int do_elif();
int do_if();
int do_xifdef();
...
/* Zero means dollar signs are punctuation.
   -$ stores 0; -traditional, stores 1.  Default is 0.
...
static int
#if PROTO_OK
error(char *msg, char *arg1, char *arg2, char *arg3)
#else
...
static int
#if PROTO_OK
warning(char *msg, char *arg1, char *arg2, char *arg3)
#else
...
int
#if PROTO_OK
error_with_line(int line, char *msg, char *arg1, char *arg2, char *arg3)
#else
error_with_line (line, msg, arg1, arg2, arg3)
int line; char *msg, arg1, arg2, arg3;
#endif
{
  int i;
  FILE_BUF *ip = NULL;
...
void
#if PROTO_OK
grow_outbuf(register FILE_BUF *obuf, register int needed)
#else
grow_outbuf (obuf, needed)
register FILE_BUF *obuf; register int needed;
#endif
{
  register U_CHAR *p;
  int minsize;
...
U_CHAR *
#if PROTO_OK
skip_to_end_of_comment(register FILE_BUF *ip, int *line_counter)
#else
skip_to_end_of_comment (ip, line_counter)
register FILE_BUF *ip; int *line_counter; /* place to remember newlines, or NULL */
#endif
{
...
U_CHAR *
#if PROTO_OK
skip_quoted_string(register U_CHAR *bp, register U_CHAR *limit,
  int start_line, int *count_newlines, int *backslash_newlines_p,
  int *eofp)
#else
skip_quoted_string (bp, limit, start_line, count_newlines, backslash_newlines_p, eofp)
register U_CHAR *bp, *limit; int start_line, *count_newlines, *backslash_newlines_p, *eofp;
#endif
{
...
int
#if PROTO_OK
hashf(register U_CHAR *name, register int len, int hashsize)
#else
hashf (name, len, hashsize)
register U_CHAR *name; register int len; int hashsize;
#endif
{
  register int r = 0;
...
HASHNODE *
#if PROTO_OK
install(U_CHAR *name, int len, enum node_type type, int hash, int ivalue,
  char *cpvalue, DEFINITION *defntion, char *present)
#else
install (name, len, type, hash, ivalue, cpvalue, defntion, present)
     U_CHAR *name;
     int len;
     enum node_type type;
     int ivalue;
        ...
        /* watch out here if sizeof (U_CHAR *) != sizeof (int) */
{
  ...
  case 'i':
	...
	break;
  case 'c':
	hp->value.cpval = cpvalue;
	break;
  case 'd':
	hp->value.defn = defntion;
	break;
  default:
	hp->value.ival = ivalue;
  }
...
void
#if PROTO_OK
dump_defn_1(U_CHAR *base, int start, int length)
#else
dump_defn_1 (base, start, length)
U_CHAR *base; int start; int length;
#endif
{
  U_CHAR *p = base + start;
...
void
#if PROTO_OK
dump_arg_n(DEFINITION *defn, int argnum)
#else
dump_arg_n (defn, argnum)
DEFINITION *defn; int argnum;
#endif
{
...
initialize_builtins ()
{
  char *cpvalue;
  DEFINITION *defn;
  install ("__LINE__", -1, T_SPECLINE, -1, 0, cpvalue, defn, "i");
  install ("__DATE__", -1, T_DATE, -1, 0, cpvalue, defn, "i");
  install ("__FILE__", -1, T_FILE, -1, 0, cpvalue, defn, "i");
  install ("__BASE_FILE__", -1, T_BASE_FILE, -1, 0, cpvalue, defn, "i");
  install ("__INCLUDE_LEVEL__", -1, T_INCLUDE_LEVEL, -1, 0, cpvalue, defn, "i");
  install ("__VERSION__", -1, T_VERSION, -1, 0, cpvalue, defn, "i");
  install ("__TIME__", -1, T_TIME, -1, 0, cpvalue, defn, "i");
  if (!traditional)
    install ("__STDC__", -1, T_CONST,  -1, STDC_VALUE, cpvalue, defn, "i");
/*  install ("__GNU__", -1, T_CONST, -1, 1, defn, "i");  */
...
#endif
{
  ...
  if (warn_trigraphs && fptr != bptr) {
    char buf[10];
    (void)sprintf(buf, "%ld", (long)((fptr - bptr) / 2));
    warning ("%s trigraph(s) encountered", buf, 0, 0);
  }
}

/*
...
#endif
{
  ...
  char *mark = (char *)obstack_alloc (&obstck,0);
  ...
  for (kt = directive_table; kt->length > 0; kt++) {
    if (kt->length == ident_length && !strncmp (
kt->name,
 ident,
 ident_length)) {
      register U_CHAR *buf;
	...
	cp = (U_CHAR *) obstack_alloc (&obstck,(bp - buf + 5)); /* room for cmd plus
						  some slop */
	buf = cp;
...
void
#if PROTO_OK
output_line_command(FILE_BUF *ip, FILE_BUF *op, int conditional,
  enum file_change_code file_change)
#else
output_line_command (ip, op, conditional, file_change)
FILE_BUF *ip, *op; int conditional; enum file_change_code file_change;
#endif
{
  int len;
...
#endif
{
  ...
  case T_BASE_FILE:
	...
	string = instack[0].fname;
      ...
      if (string)
	{
	  buf = (char *) obstack_alloc (&obstck, (3 + strlen(string)));
  ...
  case T_INCLUDE_LEVEL:
    ...
    buf = (char *) obstack_alloc (&obstck,8);	/* Eigth bytes ought to be more than enough */
    sprintf (buf, "%d", true_indepth - 1);

    break;

  case T_VERSION:
    buf = (char *) obstack_alloc(&obstck, (3 + strlen (version_string)));
    sprintf (buf, "\"%s\"", version_string);
    break;

  case T_CONST:
    buf = (char *) obstack_alloc (&obstck, 4 * sizeof (int));
    sprintf (buf, "%d", hp->value.ival);
    break;

  case T_SPECLINE:
    buf = (char *) obstack_alloc (&obstck, 10);
  ...
  case T_TIME:
    ...
    buf = (char *) obstack_alloc (&obstck,20);
    if (hp->type == T_DATE)
...
U_CHAR *buf, *limit; int output_marks;
#endif
{
  ...
  buf1 = (U_CHAR *) obstack_alloc (&obstck,length + 1);
  {
...
U_CHAR *
#if PROTO_OK
macarg1(U_CHAR *start, register U_CHAR *limit, int *depthptr,
  int *newlines, int *comments)
#else
macarg1 (start, limit, depthptr, newlines, comments)
U_CHAR *start; register U_CHAR *limit; int *depthptr, *newlines, *comments;
#endif
{
...
U_CHAR *start; int length; int newlines;
#endif
{
  register U_CHAR *ibp;
  register U_CHAR *obp;
...
char *
#if PROTO_OK
macarg(register struct argdata *argptr)
#else
macarg (argptr)
...
{ char numb[10];
  ...
  if (nargs >= 0) {
    ...
    args = (struct argdata *) obstack_alloc (&obstck,((nargs + 1) * sizeof (struct argdata)));
	...
	error ("only %s args to macro `%s'", numb, hp->name,0);
    } else if (i > nargs)
      error ("too many (%s) args to macro `%s'", numb, hp->name,0);
...
#endif
{
  ...
  char *cpvalue;
  DEFINITION *defn;
  save_defined = install ("defined", -1, T_SPEC_DEFINED, -1, 0, cpvalue, defn, "i");
...
void
#if PROTO_OK
validate_else(register U_CHAR *p)
#else
validate_else (p)
     register U_CHAR *p;
#endif
...
int
#if PROTO_OK
do_elif(U_CHAR *buf, U_CHAR *limit, FILE_BUF *op, struct directive *keyword)
#else
do_elif (buf, limit, op, keyword)
U_CHAR *buf, *limit; FILE_BUF *op; struct directive *keyword;
#endif
{
  int value;
  FILE_BUF *ip = &instack[indepth];
...
FILE_BUF *ip; int skip; enum node_type type;
#endif
{
  IF_STACK_FRAME *temp;
...
int
#if PROTO_OK
do_if(U_CHAR *buf, U_CHAR *limit, FILE_BUF *op, struct directive *keyword)
#else
...
int
#if PROTO_OK
do_xifdef( U_CHAR *buf, U_CHAR *limit, FILE_BUF *op, struct directive *keyword)
#else
...
int
#if PROTO_OK
do_else(U_CHAR *buf, U_CHAR *limit, FILE_BUF *op, struct directive *keyword)
#else
...
int
#if PROTO_OK
do_endif(U_CHAR *buf, U_CHAR *limit, FILE_BUF *op, struct directive *keyword)
#else
...
void
#if PROTO_OK
deps_output(char *string, int size)
#else
...
int fd; int *mode_pointer; long int *size_pointer;
#endif
{
  struct stat sbuf;
...
void
#if PROTO_OK
finclude(int f, char *fname, FILE_BUF *op)
#else
finclude (f, fname, op)
int f; char *fname; FILE_BUF *op;
#endif
{
  int st_mode;
  long st_size;
  ...
  if (S_ISREG (st_mode)) {
    fp->buf = (U_CHAR *) obstack_alloc (&obstck,(st_size + 2));
    fp->bufp = fp->buf;
...
get_filename:
  ...
  case '\"':
      ...
      for (fp = &instack[indepth]; fp >= instack; fp--)
	{
	  ...
	  if ((nam = fp->fname) != NULL) {
	    ...
	    if (ep != NULL) {
	      n = ep - nam;
	      dsp[0].fname = (char *) obstack_alloc (&obstck,(n + 1));
  ...
  fail:
    if (retried) {
      ...
      buf = (U_CHAR *) obstack_alloc (&obstck,(trybuf.bufp - trybuf.buf + 1));
  ...
  flen = fend - fbeg;
  fname = (char *) obstack_alloc (&obstck,(max_include_len + flen + 2));
...
U_CHAR *buf, *end; int nargs; struct arglist *arglist;
#endif
{
  DEFINITION *defn;
...
int first; U_CHAR *beg1, *beg2; int len1, len2; int last;
#endif
{
...
#endif
{
  ...
  if (sym_length == 0)
    ...
    msg = (U_CHAR *) obstack_alloc (&obstck,(sym_length + 1));
  ...
  if (*bp == '(') {
    ...
    while (*bp != ')') {
      ...
      temp = (struct arglist *) obstack_alloc (&obstck,(sizeof (struct arglist)));
      temp->name = bp;
      temp->next = arg_ptrs;
      temp->argno = argno++;
      arg_ptrs = temp;
...

  {
	...
	U_CHAR *msg;			/* what pain... */
	msg = (U_CHAR *) obstack_alloc (&obstck,(sym_length + 20));
	bcopy (symname, msg, sym_length);
	...
	warning (msg, 0, 0, 0);
      ...
      int ivalue;
      char *cpvalue;
      install (symname, sym_length, T_MACRO, hashcode, ivalue, cpvalue, defn, "d");
    }
  }
  obstack_free (&obstck,mark);
  return 0;
...
int
#if PROTO_OK
do_line(U_CHAR *buf, U_CHAR *limit, FILE_BUF *op, struct directive *keyword)
#else
...
int
#if PROTO_OK
do_undef(U_CHAR *buf, U_CHAR *limit, FILE_BUF *op, struct directive *keyword)
#else
...
int
#if PROTO_OK
do_error(U_CHAR *buf, U_CHAR *limit, FILE_BUF *op, struct directive *keyword)
#else
...
do_once ()
{
  ...
  if (ip != NULL)
    {
      struct file_name_list *temp;

      temp = (struct file_name_list *) xmalloc (sizeof (struct file_name_list));
      temp->next = dont_repeat_files;
      dont_repeat_files = temp;
      temp->fname = savestring (ip->fname);
    }
}
...
int
#if PROTO_OK
do_pragma(U_CHAR *buf, U_CHAR *limit, FILE_BUF *op, struct directive *keyword)
#else
...
int
#if PROTO_OK
do_sccs(U_CHAR *buf, U_CHAR *limit, FILE_BUF *op, struct directive *keyword)
#else
...
#endif
{
  ...
  if (*p == 0) {
    buf = (U_CHAR *) obstack_alloc (&obstck,(p - buf + 4));
    strcpy ((char *)buf, str);
    strcat ((char *)buf, " 1");
  } else if (*p != ' ') {
    error ("malformed option `-D %s'", str,0,0);
    ...
    buf = (U_CHAR *) obstack_alloc (&obstck,(2 * strlen (str) + 1));
...
#endif
{
  ...
  if (!inhibit_predefs) {
    ...
    char *p;
    p = (char *)obstack_strcpy (&obstck,predefs);
    p = (char *)(obstack_strcpy (&obstck,predefs));
    while (*p) {
      char *q;
      if (p[0] != '-' || p[1] != 'D')
  ...
  if (!S_ISREG (st_mode)) {
    ...
    /* Read a file whose size we can determine in advance.
       Actually, st_size is just an upper bound.  */
    long i;
    fp->length = 0;
