...
#endif
{
  ...
  fprintf (stderr, "    lclfile Input file containing LCL\n");
  fprintf (stderr, "    expfile Output file for expand pass\n");
  fprintf (stderr, "    ordfile Output file for order pass\n");
  fprintf (stderr, "    optfile Output file for optim pass\n");
  fprintf (stderr, "    befile Output file for backend pass\n");
    exit (1);
}
...
#endif
{
  ...
  if (i < argc) {
    lclfile = NewKey ();
    SetClpValue (lclfile, clp_string (argv[i++]), 0);
    CLP_InputFile = lclfile;
  }
  if (i < argc) {
    expfile = NewKey ();
    SetClpValue (expfile, clp_string (argv[i++]), 0);
  }
  if (i < argc) {
    ordfile = NewKey ();
    SetClpValue (ordfile, clp_string (argv[i++]), 0);
  }
  if (i < argc) {
    optfile = NewKey ();
    SetClpValue (optfile, clp_string (argv[i++]), 0);
  }
  if (i < argc) {
    befile = NewKey ();
    SetClpValue (befile, clp_string (argv[i++]), 0);
  }
