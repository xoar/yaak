/* $Id: sed.c,v 1.8 2001/12/01 19:03:36 waite Exp $ */
     ...
     char **argv;
{
  ...
  for (go = jumps; go; go = go->next)
	...
	if (!strcmp (lbl->name, go->name))
	  break;
      if (*go->name && !lbl) {
	(void)fprintf(stderr, "%s: Can't find label for jump to '%s'\n",
          myname, go->name);
	return 4;
      }
     ...
     char *name;
{
  if (*name == '-' && name[1] == '\0')
	...
	{
	  bad_input++;
	  fprintf (stderr, "%s: can't read ", myname); perror(name);
	  return;
	}
    }
...
restart:
  ...
  for (cur_cmd = vec->v, n = vec->v_length; n; cur_cmd++, n--)
	...
	case 'r':
	  {
		...
		if (ferror (cur_cmd->x.io_file)) {
		  (void)fprintf(stderr,
                    "%s: Read error on input file to 'r' command\n",
                    myname);
		  exit(4);
		}
	      }
	...
	default:
	  (void)fprintf(stderr, "%s: INTERNAL ERROR: Bad cmd %c\n",
            myname, cur_cmd->cmd);
	  exit(4);
	}
      if (end_cycle)
     ...
     struct addr *addr;
{
  switch (addr->addr_type)
    ...
    default:
      (void)fprintf(stderr, "%s: INTERNAL ERROR: bad address type", myname);
      exit(4);
    }
  return -1;
}
