...
/* $Id: global.h,v 1.12 2007/02/22 10:48:06 peter Exp $ */
...
# define typeof(c) ((!(c).IDLinternal) || ((long)(c).IDLinternal&1)?\
	((long)(c).IDLinternal):((c).IDLclassCommon->IDLhidden.TypeID))
...
extern someptr IDLListLastElem(), IDLListRetrieveFirst();
extern Boolean IDLInList();
extern int IDLListLength();
...
/* Kalle: allocate (siz+1) instead of siz because some macros 
 * (e.g.  foreachinArray(...)) accesses one behind the last array element.
 */
# define _MAX_(a,b)	((a)>(b) ? (a) : (b))
# define IDLNewArray(siz) (someptr *)(GetHeap(_MAX_(sizeof(int),sizeof(someptr))*((siz)+1)))
...
/* Kalle: deleted 'if (siz)' because of the above modification */
#define IDLInitializeArray(a,siz)	\
	{(a).array = IDLNewArray(siz); (a).size=siz; (a).length=0;}
