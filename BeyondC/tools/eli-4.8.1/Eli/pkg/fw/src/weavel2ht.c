static char RCSid[] = "$Id: weavel2ht.c,v 2.3 2013/11/12 20:50:37 profw Exp $";
/*##############################################################################
...
Changes:
   ...
   01-May-1998	WMW  Rewrite to support LaTeX and latex2html
   11-Nov-2103	WMW  Changes to address clang warnings
...
LOCVAR bool alltt;        /* True if we are inside an alltt environment.      */
 ...
 switch (ch)
   {
    ...
    case '<':  if (literal || alltt)
                  wf_chr(&f_t,ch);
               else
                  wf_wr(&f_t,"\\ensuremath{<}");
               break;
    /*   '=': is an ordinary character. */
    case '>':  if (literal || alltt)
                  wf_chr(&f_t,ch);
    ...
    case '\\': if (literal || alltt)
                  ...
                  wf_wr(&f_t,"\\ensuremath{\\backslash}");
               break;
    /*   ']': is an ordinary character. */
    case '^':  wf_wr(&f_t,"\\^{}"); break;
...
/* Write out the section name in curly braces. */
p_ty_t p_ty;
{
 wf_chr(&f_t,'{');
   ...
   switch (sn_lev(&p_ty->ty_sn))
     {
      ...
      case 5: wf_wr(&f_t,"\\paragraph"); break;
      default: as_bomb("wv_nsec:case defaulted.");
     }
 } else {
   switch (sn_lev(&p_ty->ty_sn))
     {
      case 1: wf_wr(&f_t,"\\section"); break;
      case 2: wf_wr(&f_t,"\\subsection"); break;
      case 3: wf_wr(&f_t,"\\subsubsection"); break;
      case 4: wf_wr(&f_t,"\\paragraph"); break;
      case 5: wf_wr(&f_t,"\\subparagraph"); break;
      default: as_bomb("wv_nsec:case defaulted.");
     }
 }
...
p_ty_t p_ty;
{
 ...
 sprintf(linet1,"\\vspace{%lumm}",(ulong) p_ty->ty_mm);
 wf_wl(&f_t,linet1);
}
...
p_ty_t p_ty;
{
 ...
 switch(p_ty->ty_align)
   {
    case LR_LEFT: wf_wr(&f_t,"\\begin{flushleft}");  break;
    case LR_RIGH: wf_wr(&f_t,"\\begin{flushright}"); break;
    case LR_CENT: wf_wr(&f_t,"\\begin{center}");     break;
    default: as_bomb("wv_title: Alignment switch defaulted.");
   }
 switch(p_ty->ty_font)
   {
    case FT_NORM:                                    break;
    case FT_TITL: wf_wr(&f_t,"\\textbf{\\LARGE ");    break;
    case FT_STIT: wf_wr(&f_t,"\\textbf{\\large ");    break;
    default: as_bomb("wv_title: Font switch defaulted.");
   }
 wv_ugsc(&p_ty->ty_sc);
 if (p_ty->ty_font != FT_NORM) wf_wr(&f_t,"}");
 switch(p_ty->ty_align)
   {
    case LR_LEFT: wf_wl(&f_t,"\\end{flushleft}");    break;
    case LR_RIGH: wf_wl(&f_t,"\\end{flushright}");   break;
    case LR_CENT: wf_wl(&f_t,"\\end{center}");       break;
    default: as_bomb("wv_title: Alignment switch defaulted.");
   }
}
...
p_el_t p_el;
{
 ...
 wf_wr(&f_t,"\\textnormal{"); alltt=FALSE;
 if (tr_codes!=TR_L2HTML)
    ...
    while (TRUE)
      {
       ...
       if (parnum++>1) wf_wr(&f_t,"\\textnormal{,}");
       wv_verb(*pp_scls1);
       wf_wr(&f_t,"\\textnormal{`}");
       wv_ex(*pp_exp);
       wf_wr(&f_t,"\\textnormal{'}");
 ...
 if (p_ma->ma_defn.md_isfil)
   {
    if (p_ma->ma_defn.md_isnop)
      wf_wl(&f_t,"{\\footnotesize This macro is attached to a non-product file.}");
    else
      wf_wl(&f_t,"{\\footnotesize This macro is attached to a product file.}");
    return;
   }

 if (no_calls==0)
   {
    wf_wl(&f_t,"{\\footnotesize This macro is NEVER invoked.}");
    return;
   }

 wf_wr(&f_t,"{\\footnotesize This macro is invoked in definition");
...
p_ma_t p_ma;
{
 ...
 wf_wr(&f_t,"{\\footnotesize This macro is defined in definitions ");
...
p_dc_t p_dc;
{
 ...
 wf_wl(&f_t,"\\begin{quote}\\begin{alltt}");
 alltt=TRUE;
 wv_ex(p_bp->bp_ex);
 alltt=FALSE;
 wf_wl(&f_t,"\\end{alltt}\\end{quote}");
