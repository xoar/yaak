...
/* $Id: p2c.h,v 1.15 2009/04/24 03:16:11 profw Exp $ */
...
#ifndef Static
# define Static         /* Private global funcs and vars */
#endif
...
extern intptr_t _CaseCheck  PV();
extern intptr_t _NilCheck   PV();
extern intptr_t _Escape     PP( (int) );
extern intptr_t _EscIO      PP( (int) );
...
# define tmpfile()  (fopen(tmpnam((char *)0), "w+"))
#endif
...
#define GETFBUF(f,type)    (*((__CAT__(f,_BFLAGS) == 1 &&   \
			       ((__CAT__(f,_BFLAGS) = 2),   \
				fread((Anyptr)(&__CAT__(f,_BUFFER)),  \
				      sizeof(type),1,(f)))),\
			      &__CAT__(f,_BUFFER)))
...
#define PUT(f,type)        (fwrite((Anyptr)(&__CAT__(f,_BUFFER)), \
					sizeof(type),1,(f)),  \
			    (__CAT__(f,_BFLAGS) = 0))
